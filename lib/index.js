...
async function fetchGithubVersions() {
  ...
  const result = { official: "", test: "", testTag: "" };
...
async function checkUpdate(force = false) {
  ...
  if (!npm && !ghOfficial && !ghTest) {
    ...
    const info2 = { current, latest: current, hasUpdate: false, betaLatest: current, hasBeta: false, betaTag: "" };
...
async function upgradePlugin(target) {
