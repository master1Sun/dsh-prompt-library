...
async function enforceMaxCount(maxCount, autoLearnTag) {
  ...
  const rm4 = cur.prepare("DELETE FROM prompts WHERE id = ?");
  for (const { id } of candidates.slice(0, toRemove)) rm4.run(id);
}
...
function deleteTrash(ids) {
  try {
    ...
    if (list.length === 0) return Promise.resolve(0);
    const cur = getDb();
    const rm4 = cur.prepare("DELETE FROM trash WHERE id = ?");
    let deleted = 0;
    cur.exec("BEGIN");
    try {
      for (const id of list) {
        deleted += Number(rm4.run(id).changes);
      }
      cur.exec("COMMIT");
    } catch (e) {
      cur.exec("ROLLBACK");
...
// src/host/backup.ts
import { copyFile, mkdir as mkdir2, readFile as readFile3, readdir as readdir2, rm as rm3, stat as stat2, writeFile as writeFile3 } from "node:fs/promises";
...
async function pruneBackups(retention) {
  ...
  for (const b of list.slice(keep)) {
    try {
      await rm3(join6(dir, b.name), { force: true });
    } catch {
    }
  }
}
...
async function restoreBackup(name2) {
  ...
  if (format === "db") {
    ...
    await rm3(`${dbPath()}-wal`, { force: true }).catch(() => {
    });
    await rm3(`${dbPath()}-shm`, { force: true }).catch(() => {
    });
    reopenDb();
...
async function deleteBackup(name2) {
  ...
  await rm3(join6(backupDir(), name2), { force: true });
  return true;
}
