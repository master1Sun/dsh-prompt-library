...
async function computeInactiveDays() {
  try {
    const cur = getDb();
    const row = cur.prepare("SELECT MAX(usedAt) AS lastUsed FROM usage_log").get();
    const lastUsed = row?.lastUsed;
    if (!lastUsed || lastUsed <= 0) return 0;
    const last = new Date(lastUsed);
    const todayUTC = Date.UTC((/* @__PURE__ */ new Date()).getFullYear(), (/* @__PURE__ */ new Date()).getMonth(), (/* @__PURE__ */ new Date()).getDate());
    const lastUTC = Date.UTC(last.getFullYear(), last.getMonth(), last.getDate());
    const diff = Math.floor((todayUTC - lastUTC) / (24 * 60 * 60 * 1e3));
...
function pickPhaseCopy(lang, phase, style, counter, char = "classic") {
  const group = char === "whale" ? PHASE_COPY_WHALE[phase][lang] : PHASE_COPY_CLASSIC[style][phase][lang];
...
var LEVEL_RULES = [
  ...
  { level: 6, threshold: 1e3, zh: "\u8BCD\u5E93\u5B97\u5E08", en: "Library Grandmaster" }
];
function computeLevel(totalUsage, lang, inactiveDays = 0) {
  let base = LEVEL_RULES[0];
  for (const r of LEVEL_RULES) {
    if (totalUsage >= r.threshold) base = r;
  }
  const DECAY_EVERY_DAYS = 30;
  const drop = inactiveDays > 0 ? Math.min(base.level - 1, Math.floor(inactiveDays / DECAY_EVERY_DAYS)) : 0;
  const cur = LEVEL_RULES[base.level - 1 - drop];
  const next = LEVEL_RULES.find((r) => r.level === cur.level + 1);
  const pct = next ? Math.min(100, Math.round((totalUsage - cur.threshold) / (next.threshold - cur.threshold) * 100)) : 100;
  return {
    level: cur.level,
...
function buildAssistantStatus(stats, streak, inactiveDays = 0, lang) {
  const s = stats ?? {
    total: 0,
    totalUsage: 0,
    usedCount: 0,
...
async function listBackups() {
  ...
  for (const name2 of names) {
    try {
      const s = await stat(join5(dir, name2));
...
async function pruneBackups(retention) {
  ...
  for (const b of list.slice(keep)) {
    try {
      await rm2(join5(dir, b.name), { force: true });
    } catch {
    }
  }
}
...
async function restoreBackup(name2) {
  ...
  const file = join5(backupDir(), name2);
...
async function deleteBackup(name2) {
  ...
  await rm2(join5(backupDir(), name2), { force: true });
  return true;
}
...
import { existsSync, readFileSync as readFileSync4, readdirSync } from "node:fs";
  ...
  if (kind === "workspace") {
    try {
      ...
      for (const e of entries) {
        ...
        const childPath = join6(absPath, e.name);
...
function harnessSystemSync() {
  ...
  let content = "";
  try {
    content = stripBom(readFileSync5(harnessPath(), "utf8")).trim();
  } catch {
