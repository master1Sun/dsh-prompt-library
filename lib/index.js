...
import { mkdir, readdir, readFile as readFile2, stat, writeFile as writeFile2 } from "node:fs/promises";
...
function logVersion(msg) {
  try {
    const logPath = join5(logDir(), "version.log");
...
async function restartService() {
  ...
  try {
    ...
    if (process.platform === "win32") {
      const vbs = process.env.DSH_HARNESS_VBS || join5(dshHome(), "file", "dsh-console", "dsh-harness-start.vbs");
...
import { copyFile, mkdir as mkdir2, readFile as readFile3, readdir as readdir2, rm as rm2, stat as stat2, writeFile as writeFile3 } from "node:fs/promises";
...
async function runBackup(retention, format = "db") {
  ...
  if (format === "json") {
    ...
    await writeFile3(target, JSON.stringify(data, null, 2), "utf8");
  } else {
    checkpointDb();
...
function listScopeTree() {
  ...
  if (existsSync(workspaceStorePath())) {
    try {
      raw = JSON.parse(readFileSync3(workspaceStorePath(), "utf8"));
    } catch {
    }
  }
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (method === "POST" && segments[0] === "export" && segments[1] === "save") {
        ...
        let n = 1;
        while (existsSync2(join8(dir, finalName))) {
          finalName = ext ? `${base} (${n}).${ext}` : `${base} (${n})`;
          n++;
        }
        const target = join8(dir, finalName);
        await writeFile4(target, file.content, "utf8");
      ...
      if (method === "GET" && segments[0] === "assets" && segments[1] === "whale" && segments.length === 2) {
        try {
          ...
          const buf = await readFile4(fileURLToPath(fileUrl));
          res.writeHead(200, {
...
async function ensureHarnessFile() {
  try {
    await readFile5(harnessPath(), "utf8");
  } catch {
    await mkdir4(dirname4(harnessPath()), { recursive: true });
...
var manualZh = [
  ...
  "",
  "\u3010\u516B\u3001\u4EBA\u683C\uFF08\u5B9E\u9A8C\u5BA4\u529F\u80FD\uFF09\u3011",
  "- \u4EBA\u683C\u6B63\u6587\u5B58\u4E8E\u6570\u636E\u5E93\uFF08prompts.db\uFF09\uFF1A\u9ED8\u8BA4\u4EBA\u683C\u5B58\u4E8E meta \u8868\u3001\u81EA\u5B9A\u4E49\u4EBA\u683C\u5B58\u4E8E personas \u8868\uFF0C\u805A\u5408\u8EAB\u4EFD\u3001\u6027\u683C/\u8BED\u6C14\u3001\u5DE5\u4F5C\u89C4\u8303\uFF1B",
  "- \u5185\u5BB9\u9700\u624B\u52A8\u586B\u5199\u7EF4\u62A4\uFF0C\u5F62\u6210\u7A33\u5B9A\u7684\u52A9\u624B\u4EBA\u683C\uFF1B\u7F3A\u5931\u65F6\u81EA\u52A8\u5199\u5165\u9ED8\u8BA4\u6A21\u677F\uFF1B",
...
var manualEn = [
  ...
  "- Personality content is stored in the database (prompts.db): the default persona lives in the meta table, custom personas in the personas table, combining identity, tone, and work guidelines;",
  "- Fill it in manually to form a stable assistant personality; a missing one is rebuilt with the default template automatically;",
