...
import { mkdir as mkdir2, readdir, readFile as readFile3, writeFile as writeFile3 } from "node:fs/promises";
...
async function exportPromptsAsSkills(entries) {
  const items = [];
  const errors = [];
  for (let i = 0; i < entries.length; i++) {
    const entry = entries[i];
...
import { copyFile, mkdir as mkdir3, readFile as readFile4, readdir as readdir2, rm as rm2, stat, writeFile as writeFile4 } from "node:fs/promises";
...
async function listBackups() {
  ...
  for (const name2 of names) {
    try {
      const s = await stat(join6(dir, name2));
...
async function ensureHarnessFile() {
  try {
    ...
    await mkdir4(dirname5(harnessPath()), { recursive: true });
    await writeFile5(harnessPath(), DEFAULT_HARNESS, "utf8");
  }
}
