...
import { existsSync, mkdirSync as mkdirSync4, readFileSync as readFileSync4, readdirSync, writeFileSync } from "node:fs";
...
function writeIssueMarkdown(issue) {
  try {
    ...
    mkdirSync4(join5(newspapersDir(), issue.lang), { recursive: true });
...
async function getIssue(date, lang, settings) {
  ...
  if (existsSync(mdPath)) {
    return parseIssueMarkdown(date, L, readFileSync4(mdPath, "utf8"));
  }
  if (date !== today) {
  ...
  const [reportZh, reportEn] = await Promise.all([
    statsText ? generateDailyReport(statsText, settings, "zh") : void 0,
    statsText ? generateDailyReport(statsText, settings, "en") : void 0
  ]);
  const versions = [
...
import { existsSync as existsSync2, readFileSync as readFileSync5, readdirSync as readdirSync2 } from "node:fs";
...
function resolvePersonaForPath(cwd) {
  ...
  const normals = new Map(
    listScopeBindings().map((b) => [normalizeScopePath(b.path), b.personaId])
  );
...
function listScopeTree() {
  ...
  if (existsSync2(workspaceStorePath())) {
    try {
      raw = JSON.parse(readFileSync5(workspaceStorePath(), "utf8"));
    } catch {
    }
  }
...
function harnessSystemSync() {
  ...
  let content = "";
  try {
    content = stripBom(readFileSync6(harnessPath(), "utf8")).trim();
  } catch {
