...
import { cp as cp2, mkdir as mkdir5, readFile as readFile6, readdir as readdir3, rename, rm as rm5, stat as stat4, writeFile as writeFile6 } from "node:fs/promises";
...
var PINNED_SLUGS = ["dsh-file-workbench-lib", "dsh-prompt-library"];
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        ...
        try {
          ...
          const text = await readFile6(full, "utf8");
          const lines = text.split("\n");
          let perFile = 0;
...
`);
      ...
      if (method === "POST" && segments[0] === "preview" && segments[1] === "save" && segments.length === 2) {
        ...
        try {
          await writeFile6(path, data.content, "utf8");
      ...
      if (method === "POST" && segments[0] === "preview" && segments[1] === "rename" && segments.length === 2) {
        ...
        try {
          const newPath = join8(basename3(p) === p ? "" : p.slice(0, p.length - basename3(p).length), b.name);
      ...
      if (method === "POST" && segments[0] === "preview" && segments[1] === "mkdir" && segments.length === 2) {
        ...
        try {
          await mkdir5(join8(dir, b.name), { recursive: false });
          return json(res, 200, { ok: true, data: { success: true } });
        } catch (err) {
          return json(res, 500, {
            ok: false,
      ...
      if (method === "POST" && segments[0] === "preview" && segments[1] === "newfile" && segments.length === 2) {
        ...
        try {
          await writeFile6(join8(dir, b.name), "", "utf8");
          return json(res, 200, { ok: true, data: { success: true } });
        } catch (err) {
          return json(res, 500, {
            ok: false,
...
`);
      ...
      if (method === "POST" && segments[0] === "preview" && segments[1] === "move" && segments.length === 2) {
        ...
        try {
          ...
          const dest = join8(targetDir, name2);
      ...
      if (method === "POST" && segments[0] === "preview" && segments[1] === "copy" && segments.length === 2) {
        ...
        try {
          const dest = join8(targetDir, basename3(p));
