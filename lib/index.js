// src/host/services/ai.ts
import { BlockAssembler, createUserMessage } from "@deepseek-ai/dsh-llm";

// src/host/services/store.ts
...
// src/host/services/character.ts
...
var MASTER_PACKAGE_URL = `https://api.github.com/repos/${GITHUB_REPO}/contents/package.json?ref=master`;
...
function buildVersionLogCopy(lang) {
  ...
  if (zh) {
    return {
      check: (cur, npm, git, exp) => `\u7248\u672C\u68C0\u67E5 \u5F53\u524D=${cur} npm=${npm} git=${git} \u4F53\u9A8C\u8BA1\u5212=${exp ? on : off}`,
  ...
  return {
    check: (cur, npm, git, exp) => `Version check current=${cur} npm=${npm} git=${git} experience=${exp ? on : off}`,
...
function fetchRawText(url, headers = {}) {
  return new Promise((resolve, reject) => {
    const req = httpsGet(url, { headers }, (res) => {
      const code = res.statusCode ?? 0;
      if (code < 200 || code >= 300) {
...
async function checkUpdate(force = false) {
  ...
  if (betaEnabled) {
    const info2 = gitUpdate ? { current, latest: ghTest, hasUpdate: true, betaLatest: current, hasBeta: false, gitTag: ghTag, betaTag: "" } : npmUpdate ? { current, latest: npm, hasUpdate: true, betaLatest: current, hasBeta: false, gitTag: "", betaTag: "" } : { current, latest: current, hasUpdate: false, betaLatest: current, hasBeta: false, gitTag: "", betaTag: "" };
    logVersion(vlog.expResult(info2.latest));
    cache = { at: now, info: info2, ttl: CACHE_MS };
    return info2;
  }
  const info = npmUpdate ? { current, latest: npm, hasUpdate: true, betaLatest: npm, hasBeta: true, gitTag: "", betaTag: "" } : { current, latest: current, hasUpdate: false, betaLatest: current, hasBeta: false, gitTag: "", betaTag: "" };
