...
function autoLearn(body, tag, skipEnrich) {
  try {
    ...
    if (trashHit) {
      ...
      void continueEnrich(prompt2, !!skipEnrich);
      emitDataChanged();
      return Promise.resolve(prompt2);
    }
...
function seedPointsLedger(cur) {
  try {
    ...
    cap2("learn", learnPoints > 0 ? Math.ceil(learnPoints / POINTS_WEIGHT.learn) : 0);
...
function listPersonas() {
  try {
    const rows = getDb().prepare("SELECT id, name, enabled, createdAt, updatedAt FROM personas ORDER BY createdAt ASC").all();
...
function getPersona(id) {
  try {
    const row = getDb().prepare("SELECT id, name, enabled, createdAt, updatedAt FROM personas WHERE id = ?").get(id);
    return row ? personaFromRow(row) : void 0;
  } catch {
    return void 0;
  }
}
function createPersona(id, name2) {
  const now = Date.now();
  const db_ = getDb();
  db_.prepare("INSERT INTO personas (id, name, enabled, createdAt, updatedAt) VALUES (?, ?, 1, ?, ?)").run(id, name2, now, now);
  return { id, name: name2, enabled: true, createdAt: now, updatedAt: now };
}
function updatePersonaMeta(id, patch) {
  ...
  getDb().prepare("UPDATE personas SET name = ?, enabled = ?, updatedAt = ? WHERE id = ?").run(next.name, next.enabled ? 1 : 0, next.updatedAt, id);
  return true;
}
...
import { readFileSync as readFileSync2, rmSync, statSync } from "node:fs";
...
async function ensureSoulFile() {
  const path = soulPath();
  try {
    const current = stripBom(await readFile2(path, "utf8"));
    if (current.trim() === DEFAULT_SOUL.trim()) {
      await writeFile2(path, stripBom(DEFAULT_PERSONA_SOUL), "utf8");
    }
  } catch {
...
async function ensurePersonaSoul(personaId) {
  ...
  try {
    ...
    await mkdir(dirname2(path), { recursive: true });
    await writeFile2(path, stripBom(DEFAULT_SOUL), "utf8");
  }
}
async function readSoulDoc() {
  try {
    return stripBom(await readFile2(soulPath(), "utf8")).trim();
  } catch {
    return "";
  }
}
async function readPersonaSoul(personaId) {
  ...
  try {
    return stripBom(await readFile2(path, "utf8")).trim();
  } catch {
    return "";
  }
}
async function writePersonaSoul(content, personaId) {
  ...
  const path = id ? personaSoulPath(id) : soulPath();
  await mkdir(dirname2(path), { recursive: true });
  await writeFile2(path, stripBom(content), "utf8");
}
...
var soulCacheByPath = /* @__PURE__ */ new Map();
...
function soulSystemSync(personaId) {
  ...
  let content = "";
  try {
    content = stripBom(readFileSync2(path, "utf8")).trim();
  } catch {
...
import { mkdir as mkdir2, readdir, readFile as readFile3, stat, writeFile as writeFile3 } from "node:fs/promises";
import { basename, join as join3 } from "node:path";

// src/host/services/session-prompts/session-prompts.ts
import { randomUUID as randomUUID2 } from "node:crypto";
import { readFileSync as readFileSync3, mkdirSync as mkdirSync3, writeFileSync, rmSync as rmSync2 } from "node:fs";
...
function ensureDir() {
  try {
    mkdirSync3(sessionPromptsDir(), { recursive: true });
  } catch {
  }
}
function readPromptBody(id) {
  try {
    return stripBom(readFileSync3(sessionPromptPath(id), "utf8")).trim();
  } catch {
    return "";
  }
}
function writePromptBody(id, body) {
  try {
...
function updateSessionPrompt(id, patch) {
  ...
  if (patch.body !== void 0) writePromptBody(id, next.body);
  return next;
}
function deleteSessionPrompt(id) {
  ...
  try {
    rmSync2(sessionPromptPath(id), { force: true });
  } catch {
  }
  return true;
}
  ...
  const walk = async (d) => {
    ...
    for (const ent of entries) {
      if (ent.isDirectory()) {
        ...
        try {
          const raw = await readFile3(join3(d, ent.name), "utf8");
          const parsed = parseSkillFile(raw);
          const body = parsed.body.trim();
          if (!body) continue;
...
async function scanSkillRootForToggles(root) {
  ...
  for (const name2 of names) {
    ...
    let raw;
    try {
      raw = await readFile3(join3(root, name2, "SKILL.md"), "utf8");
    } catch {
      continue;
    }
    const parsed = parseSkillFile(raw);
...
import { appendFileSync as appendFileSync2, mkdirSync as mkdirSync4, readFileSync as readFileSync4 } from "node:fs";
...
async function restartService() {
  ...
  try {
    ...
    if (process.platform === "win32") {
      const vbs = process.env.DSH_HARNESS_VBS || join4(dshHome(), "file", "dsh-console", "dsh-harness-start.vbs");
...
// src/host/services/assistant/phrases.ts
var STYLE_KEYWORDS = {
  code: [
    "\u4EE3\u7801",
    "\u7F16\u7A0B",
...
var PHASE_COPY_WHALE = {
  ...
  thinking: {
    ...
    en: [
      ...
      "Gathering sparks~",
      "Frowning, analysing carefully",
      "Left brain and right brain in a huddle",
      "The answer is surfacing",
      "Let me untangle the logic",
      "Letting it all sink in",
      "No rush, I'm thinking",
      "Brain's spinning up",
      "Let me connect the dots",
      "Thoughts racing along",
      "Little head at full speed",
      "Let me chew on this",
      "Browsing the library in my head",
      "Let me chew over this question",
      "Brain's brewing coffee, almost done",
      "A thought-fish just swam by",
      "Let me peek at the trick here",
      "Unravelling the logic chain",
      "Sorting and storing thoughts",
      "Hmm? Now that's interesting\u2026",
      "Letting the idea settle",
      "Thoughts scrolling by at light speed"
    ]
  },
  tool: {
    zh: [
  ...
  review: {
    ...
    en: [
      "All done!",
      "Nailed it, wrapping up~",
      "Task complete, yay!",
      "This round, flawlessly done",
      "Made it to the finish",
      "Done! Pet me as a reward?",
      "Handing it in! Next, please",
      "All set, beautiful finish",
      "Got it! High five~",
      "Solid, perfect score",
      "Done, time for water",
      "All wrapped up, spinning in circles",
      "That round, we were the perfect team",
  ...
  failed: {
    ...
    en: [
      "Execution failed",
      "Oops, got stuck mid-way",
      "Couldn't finish this step",
      "Tripped over a little stone",
      "Took a tumble, rubbing my knees",
      "Hit a snag, catch our breath and retry",
      "Tool execution failed",
      "Tool's throwing a tantrum, let's soothe it",
      "Oops, the tool fell off track",
      "This tool's being stubborn today",
      "The tool flipped over \u2014 pick it up and go on",
      "Didn't get through, try again",
      "Tool: I'm on strike for three seconds",
      "This step stumbled, no worries"
    ]
  }
};
function pickPhaseCopy(lang, phase, style, counter, char = "classic") {
  const group = char === "whale" || char === "dshpet" ? PHASE_COPY_WHALE[phase][lang] : PHASE_COPY_CLASSIC[style][phase][lang];
...
var ActivityMachine = class {
  ...
  render(lang, char = "classic") {
    ...
    const phase = doneSettled || failedSettled ? "idle" : this.phase;
    return {
      phase,
...
function getActivity(lang = "zh", char = "classic") {
...
// src/host/services/assistant/gamification.ts
var RARITY_POINTS = {
  common: 1,
  rare: 3,
  epic: 5,
...
var ACHIEVEMENT_RULES = [
  ...
  },
  {
    ...
    enTitle: "Seedling",
    zhDesc: "\u8BCD\u5E93\u6536\u85CF\u6EE1 5 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Collect 5 prompts in the library",
  ...
  },
  {
    ...
    zhDesc: "\u8FDE\u7EED 3 \u5929\u4F7F\u7528\u8BCD\u5E93",
    enDesc: "Use the library 3 days in a row",
  ...
  },
  {
    ...
    zhTitle: "\u81EA\u52A8\u62FE\u53D6",
    enTitle: "Auto Pickup",
    zhDesc: "\u81EA\u52A8\u5B66\u4E60\u5165\u5E93 1 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Auto-learn 1 prompt into the library",
  ...
  },
  {
    ...
    zhDesc: "\u7D2F\u8BA1\u7528\u8FC7 20 \u6761\u4E0D\u540C\u7684\u63D0\u793A\u8BCD",
    enDesc: "Use 20 distinct prompts in total",
  ...
  },
  {
    ...
    zhTitle: "\u4E66\u67B6\u4E0A\u65B0",
    enTitle: "New Shelves",
    zhDesc: "\u8BCD\u5E93\u6536\u85CF\u6EE1 15 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Collect 15 prompts in the library",
  ...
  },
  {
    ...
    zhTitle: "\u5C0F\u6807\u7B7E\u5BB6",
    enTitle: "Mini Tagger",
    zhDesc: "\u5355\u4E00\u6807\u7B7E\u4E0B\u6536\u5F55 3 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Keep 3 prompts under one tag",
  ...
  },
  {
    ...
    zhDesc: "\u8FD1 7 \u5929\u6709 5 \u6761\u63D0\u793A\u8BCD\u88AB\u4F7F\u7528",
    enDesc: "Use 5 different prompts within 7 days",
  ...
  },
  {
    ...
    zhTitle: "\u70ED\u529B\u5F00\u573A",
    enTitle: "Warm Start",
    zhDesc: "\u8FD1 7 \u5929\u7D2F\u8BA1\u4F7F\u7528 15 \u6B21\u63D0\u793A\u8BCD",
    enDesc: "Use prompts 15 times within 7 days",
  ...
  },
  {
    ...
    zhDesc: "\u7D2F\u8BA1\u7528 AI \u5B8C\u5584 3 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Polish 3 prompts with AI in total",
  ...
  },
  {
    ...
    zhDesc: "\u8FD1 7 \u5929\u65B0\u589E 3 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Add 3 prompts within 7 days",
  ...
  // ── 稀有档（12）──
  {
    ...
    zhDesc: "\u7D2F\u8BA1\u4F7F\u7528 60 \u6B21\u63D0\u793A\u8BCD",
    enDesc: "Use prompts 60 times in total",
  ...
  },
  {
    ...
    zhTitle: "\u4ECE\u5BB9\u4E0D\u8FEB",
    enTitle: "Unflappable",
    zhDesc: "\u7D2F\u8BA1\u4F7F\u7528 120 \u6B21\u63D0\u793A\u8BCD",
    enDesc: "Use prompts 120 times in total",
  ...
  },
  {
    ...
    zhDesc: "\u8BCD\u5E93\u6536\u85CF\u6EE1 120 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Collect 120 prompts in the library",
  ...
  },
  {
    ...
    enTitle: "A Fortnight",
    zhDesc: "\u8FDE\u7EED 14 \u5929\u4F7F\u7528\u8BCD\u5E93",
    enDesc: "Use the library 14 days in a row",
  ...
  },
  {
    ...
    zhTitle: "\u56DB\u5904\u5F00\u82B1",
    enTitle: "Widespread",
    zhDesc: "\u8FD1 30 \u5929\u7528\u8FC7 15 \u6761\u4E0D\u540C\u7684\u63D0\u793A\u8BCD",
    enDesc: "Use 15 different prompts within 30 days",
  ...
  },
  {
    ...
    target: 15,
    zhTitle: "\u535A\u91C7\u4F17\u957F",
    enTitle: "Encyclopedic",
    zhDesc: "\u81EA\u52A8\u5B66\u4E60\u5165\u5E93 15 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Auto-learn 15 prompts into the library",
  ...
  },
  {
    ...
    zhDesc: "\u7D2F\u8BA1\u7528\u8FC7 80 \u6761\u4E0D\u540C\u7684\u63D0\u793A\u8BCD",
    enDesc: "Use 80 distinct prompts in total",
  ...
  },
  {
    ...
    zhDesc: "\u7D2F\u8BA1\u7528 AI \u5B8C\u5584 50 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Polish 50 prompts with AI in total",
  ...
  },
  {
    ...
    zhTitle: "\u72EC\u5F53\u4E00\u9762",
    enTitle: "Standalone",
    zhDesc: "\u7D2F\u8BA1\u4F7F\u7528 600 \u6B21\u63D0\u793A\u8BCD",
    enDesc: "Use prompts 600 times in total",
  ...
  },
  {
    ...
    zhDesc: "\u8BCD\u5E93\u6536\u85CF\u6EE1 200 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Collect 200 prompts in the library",
  ...
  },
  {
    ...
    zhDesc: "\u8FDE\u7EED 60 \u5929\u4F7F\u7528\u8BCD\u5E93",
    enDesc: "Use the library 60 days in a row",
  ...
  },
  {
    ...
    zhTitle: "\u9AD8\u4EA7\u8FBE\u4EBA",
    enTitle: "Prod Heavy",
    zhDesc: "\u8FD1 30 \u5929\u65B0\u589E 50 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Add 50 prompts within 30 days",
  ...
  },
  {
    ...
    target: 60,
    zhTitle: "\u5B66\u6D77\u65E0\u6DAF",
    enTitle: "Endless Study",
    zhDesc: "\u81EA\u52A8\u5B66\u4E60\u5165\u5E93 60 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Auto-learn 60 prompts into the library",
  ...
  },
  {
    ...
    zhTitle: "\u5E7F\u5F00\u8A00\u8DEF",
    enTitle: "Broad Reach",
    zhDesc: "\u7D2F\u8BA1\u7528\u8FC7 300 \u6761\u4E0D\u540C\u7684\u63D0\u793A\u8BCD",
    enDesc: "Use 300 distinct prompts in total",
  ...
  },
  {
    ...
    zhTitle: "\u5386\u4E45\u5F25\u65B0",
    enTitle: "Time-Tested",
    zhDesc: "\u7D2F\u8BA1\u7528\u8FC7 450 \u6761\u4E0D\u540C\u7684\u63D0\u793A\u8BCD",
    enDesc: "Use 450 distinct prompts in total",
  ...
  },
  {
    ...
    zhDesc: "\u7D2F\u8BA1\u7528 AI \u5B8C\u5584 120 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Polish 120 prompts with AI in total",
  ...
  },
  {
    ...
    zhDesc: "AI \u5B8C\u5584\u7684\u6280\u80FD\u5360\u6BD4\u8FBE 60%",
    enDesc: "60% of your prompts polished by AI",
  ...
  },
  {
    ...
    zhDesc: "\u8FD1 7 \u5929\u7D2F\u8BA1\u4F7F\u7528 60 \u6B21\u63D0\u793A\u8BCD",
    enDesc: "Use prompts 60 times within 7 days",
  ...
  },
  {
    ...
    zhDesc: "\u56DE\u6536\u7AD9\u7D2F\u8BA1\u6709 25 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Keep 25 prompts in the recycle bin",
  ...
  },
  {
    ...
    zhTitle: "\u4E13\u4E1A\u6DF1\u8015",
    enTitle: "Deep Focus",
    zhDesc: "\u5355\u4E00\u6807\u7B7E\u4E0B\u6536\u5F55 20 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Keep 20 prompts under one tag",
  ...
  },
  {
    ...
    zhDesc: "\u8FD1 7 \u5929\u7528 AI \u5B8C\u5584 30 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Polish 30 prompts with AI within 7 days",
  ...
  },
  {
    ...
    zhDesc: "\u8FD1 7 \u5929\u6700\u5E38\u7528\u6280\u80FD\u5355\u65E5\u4F7F\u7528\u8D85 35 \u6B21",
    enDesc: "A top prompt is used 35+ times in 7 days",
  ...
  },
  {
    ...
    zhTitle: "\u6325\u6D12\u81EA\u5982",
    enTitle: "Effortless",
    zhDesc: "\u7D2F\u8BA1\u4F7F\u7528 900 \u6B21\u63D0\u793A\u8BCD",
    enDesc: "Use prompts 900 times in total",
  ...
  },
  {
    ...
    zhDesc: "\u8BCD\u5E93\u6536\u85CF\u6EE1 400 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Collect 400 prompts in the library",
  ...
  },
  {
    ...
    zhDesc: "\u8FDE\u7EED 30 \u5929\u4F7F\u7528\u8BCD\u5E93",
    enDesc: "Use the library 30 days in a row",
  ...
  },
  {
    ...
    target: 100,
    zhTitle: "\u5B66\u65E0\u6B62\u5883",
    enTitle: "No End to Learning",
    zhDesc: "\u81EA\u52A8\u5B66\u4E60\u5165\u5E93 100 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Auto-learn 100 prompts into the library",
  ...
  },
  {
    ...
    zhDesc: "\u6536\u85CF\u6280\u80FD\u7D2F\u8BA1 2.5 \u4E07\u5B57",
    enDesc: "Stockpile 25,000 characters of prompts",
    progress: (s) => cap(s.totalBodyLength, 25e3)
  },
  // ── 传说档（16）──
  {
    id: "use_1000",
    ...
    zhDesc: "\u7D2F\u8BA1\u4F7F\u7528 1000 \u6B21\u63D0\u793A\u8BCD",
    enDesc: "Use prompts 1000 times in total",
    progress: (s) => cap(s.totalUsage, 1e3)
  },
  {
    id: "use_2000",
    rarity: "legendary",
    ...
    enDesc: "Use prompts 2000 times in total",
  ...
  },
  {
    ...
    zhDesc: "\u8BCD\u5E93\u6536\u85CF\u6EE1 600 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Collect 600 prompts in the library",
  ...
  },
  {
    ...
    zhDesc: "\u8BCD\u5E93\u6536\u85CF\u6EE1 900 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Collect 900 prompts in the library",
  ...
  },
  {
    ...
    zhDesc: "\u8FDE\u7EED 150 \u5929\u4F7F\u7528\u8BCD\u5E93",
    enDesc: "Use the library 150 days in a row",
  ...
  },
  {
    ...
    zhDesc: "\u7D2F\u8BA1\u7528\u8FC7 500 \u6761\u4E0D\u540C\u7684\u63D0\u793A\u8BCD",
    enDesc: "Use 500 distinct prompts in total",
  ...
  },
  {
    ...
    zhDesc: "\u7D2F\u8BA1\u7528\u8FC7 800 \u6761\u4E0D\u540C\u7684\u63D0\u793A\u8BCD",
    enDesc: "Use 800 distinct prompts in total",
  ...
  },
  {
    ...
    zhDesc: "\u7D2F\u8BA1\u7528 AI \u5B8C\u5584 250 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Polish 250 prompts with AI in total",
  ...
  },
  {
    ...
    zhTitle: "\u5B66\u8D2F\u53E4\u4ECA",
    enTitle: "Era Scholar",
    zhDesc: "\u81EA\u52A8\u5B66\u4E60\u5165\u5E93 150 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Auto-learn 150 prompts into the library",
  ...
  },
  {
    ...
    zhDesc: "\u6536\u85CF\u6280\u80FD\u7D2F\u8BA1 5 \u4E07\u5B57",
    enDesc: "Stockpile 50,000 characters of prompts",
    progress: (s) => cap(s.totalBodyLength, 5e4)
  },
  {
    id: "trash_50",
    rarity: "legendary",
    ...
    zhDesc: "\u56DE\u6536\u7AD9\u7D2F\u8BA1\u6709 50 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Keep 50 prompts in the recycle bin",
  ...
  },
  {
    ...
    zhDesc: "\u8FD1 30 \u5929\u65B0\u589E 100 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Add 100 prompts within 30 days",
  ...
  },
  {
    ...
    zhDesc: "\u8FDE\u7EED 180 \u5929\u4F7F\u7528\u8BCD\u5E93",
    enDesc: "Use the library 180 days in a row",
  ...
  },
  {
    ...
    zhDesc: "AI \u5B8C\u5584\u7684\u6280\u80FD\u5360\u6BD4\u8FBE 80%",
    enDesc: "80% of your prompts polished by AI",
  ...
  // ── 神话档（8）最高难度，对应最强的大阿卡纳（高塔/星星/月亮/太阳/审判/世界/恶魔/死神）──
  {
    ...
    zhDesc: "\u7D2F\u8BA1\u4F7F\u7528 10000 \u6B21\u63D0\u793A\u8BCD",
    enDesc: "Use prompts 10,000 times in total",
    progress: (s) => cap(s.totalUsage, 1e4)
  },
  {
    id: "collector_1500",
    rarity: "myth",
    ...
    zhDesc: "\u8BCD\u5E93\u6536\u85CF\u6EE1 1500 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Collect 1,500 prompts in the library",
  ...
  },
  {
    ...
    zhTitle: "\u535A\u95FB\u5F3A\u8BC6",
    enTitle: "Wide-Eyed",
    zhDesc: "\u7D2F\u8BA1\u7528\u8FC7 3000 \u6761\u4E0D\u540C\u7684\u63D0\u793A\u8BCD",
    enDesc: "Use 3,000 distinct prompts in total",
    progress: (s) => cap(s.usedCount, 3e3)
  },
  {
    id: "ai_1000",
    rarity: "myth",
    ...
    zhDesc: "\u7D2F\u8BA1\u7528 AI \u5B8C\u5584 1000 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Polish 1,000 prompts with AI in total",
    progress: (s) => cap(s.aiRefinedCount, 1e3)
  },
  {
    id: "word_100000",
    rarity: "myth",
    ...
    zhDesc: "\u6536\u85CF\u6280\u80FD\u7D2F\u8BA1 10 \u4E07\u5B57",
    enDesc: "Stockpile 100,000 characters of prompts",
    progress: (s) => cap(s.totalBodyLength, 1e5)
  },
  {
    id: "streak_1095",
    rarity: "myth",
    ...
    zhDesc: "\u8FDE\u7EED 1095 \u5929\u4F7F\u7528\u8BCD\u5E93",
    enDesc: "Use the library 1,095 days in a row",
  ...
  },
  {
    ...
    zhDesc: "\u8FD1 7 \u5929\u7D2F\u8BA1\u4F7F\u7528 1000 \u6B21\u63D0\u793A\u8BCD",
    enDesc: "Use prompts 1,000 times within 7 days",
    progress: (s) => cap(s.usedIn7Days > 0 ? s.topUsed7.reduce((a, b) => a + b.count, 0) : 0, 1e3)
  },
  {
    id: "learner_1000",
    rarity: "myth",
    target: 1e3,
    zhTitle: "\u6D77\u7EB3\u767E\u5DDD",
    enTitle: "All Rivers Flow",
    zhDesc: "\u81EA\u52A8\u5B66\u4E60\u5165\u5E93 1000 \u6761\u63D0\u793A\u8BCD",
    enDesc: "Auto-learn 1,000 prompts into the library",
    progress: (s) => cap(s.autoLearnedCount, 1e3)
  }
];
function computeAchievements(stats, streak, lang) {
...
function buildAssistantStatus(stats, streak, lang, points) {
  const s = stats ?? {
    total: 0,
    totalUsage: 0,
    usedCount: 0,
...
function buildAchievementNews(stats, streak, lang) {
  const s = stats ?? {
    total: 0,
    totalUsage: 0,
    usedCount: 0,
  ...
  const pending = computeAchievements(s, streak, lang).filter((a) => !a.achieved).sort((a, b) => b.progress - a.progress);
  const items = [];
...
var VERSION_NOTES = [
  {
    ...
    date: "2026-08-28",
    zh: {
      title: "\u7B49\u7EA7\u914D\u8272 \xB7 \u6280\u80FD\u7BA1\u7406 \xB7 \u6570\u636E\u7BA1\u7406\u6574\u5408 \xB7 \u56FE\u6807\u7EDF\u4E00",
      items: [
    ...
    en: {
      title: "Level colors \xB7 Skill manager \xB7 Data management merge \xB7 Unified icon",
      items: [
  ...
  },
  {
    ...
    date: "2026-08-28",
    zh: {
      title: "\u4F1A\u8BDD\u6280\u80FD\u6CE8\u5165 \xB7 \u5BFC\u5165\u6807\u7B7E\u4E0B\u62C9 \xB7 \u516C\u544A\u4F18\u5316",
      items: [
    ...
    en: {
      title: "Session skill injection \xB7 Import tag dropdown \xB7 Announcement polish",
      items: [
  ...
  },
  {
    ...
    date: "2026-08-27",
    zh: {
      title: "\u6BCF\u65E5\u65E5\u62A5 \xB7 \u9CB8\u9C7C\u5F62\u8C61 \xB7 \u6210\u5C31\u4E0E\u6D1E\u5BDF\u5347\u7EA7",
      items: [
    ...
    en: {
      title: "Daily report \xB7 Whale look \xB7 Achievements & insights",
      items: [
  ...
  },
  {
    ...
    date: "2026-08-26",
    zh: {
      title: "\u54C8\u58EB\u5947\u5F62\u8C61 \xB7 \u6EE1\u7EA7\u89E3\u9501 \xB7 \u5BFC\u5165\u5F52\u96F6 \xB7 \u4EBA\u683C\u7559\u767D",
      items: [
    ...
    en: {
      title: "Husky look \xB7 Max-level unlock \xB7 Fresh ledger \xB7 Blank personae",
      items: [
  ...
  },
  {
    ...
    date: "2026-08-25",
    zh: {
      title: "\u67E5\u770B\u8BE6\u60C5 \xB7 \u5168\u5C40\u4E3B\u9898\u63D0\u793A \xB7 \u5361\u7247\u5217\u8868",
      items: [
    ...
    en: {
      title: "View details \xB7 Themed tooltips \xB7 Card lists",
      items: [
  ...
  },
  {
    ...
    date: "2026-08-25",
    zh: {
      title: "\u5BFC\u51FA\u6280\u80FD AI \u751F\u6210 \xB7 \u81EA\u5B9A\u4E49 JSON \u5BFC\u5165",
      items: [
    ...
    en: {
      title: "Export skills with AI \xB7 Custom JSON import",
      items: [
  ...
  },
  {
    ...
    en: {
      title: "Statistics view \xB7 Tag filtering \xB7 UI polish",
      items: [
...
// src/host/services/update/daily.ts
import { existsSync, mkdirSync as mkdirSync5, readFileSync as readFileSync5, readdirSync, writeFileSync as writeFileSync2 } from "node:fs";
...
function issueMdPath(date, lang) {
  return join5(newspapersDir(), lang, `${date}.md`);
}
function beijingNow() {
...
function formatFriendlyDate(dateKey, lang) {
  try {
...
function writeIssueMarkdown(issue) {
  try {
    const { at } = beijingNow();
    const en = issue.lang === "en";
    const lines = [];
    lines.push(`# ${en ? "The Prompt Daily" : "\u8BCD\u5E93\u65E5\u62A5"} \xB7 ${formatFriendlyDate(issue.date, issue.lang)}`);
    lines.push("");
    ...
    lines.push("");
    lines.push(`## \u{1F4F0} ${en ? "Daily Report" : "\u6BCF\u65E5\u65E5\u62A5"}`);
    ...
    mkdirSync5(join5(newspapersDir(), issue.lang), { recursive: true });
    writeFileSync2(issueMdPath(issue.date, issue.lang), `${lines.join("\n")}
`, "utf8");
  } catch {
  }
}
function parseIssueMarkdown(date, lang, content) {
  const report = [];
  const news = [];
  let section = null;
...
async function getIssue(date, lang, settings) {
  ...
  if (existsSync(mdPath)) {
    const cached = parseIssueMarkdown(date, L, readFileSync5(mdPath, "utf8"));
...
function listIssueDates() {
  ...
  for (const lang of ["zh", "en"]) {
    try {
      const dir = join5(newspapersDir(), lang);
      for (const name2 of readdirSync(dir)) {
        if (/^\d{4}-\d{2}-\d{2}\.md$/.test(name2)) dates.add(name2.slice(0, 10));
      }
    } catch {
    }
  }
...
// src/host/services/data/backup.ts
import { copyFile, mkdir as mkdir3, readFile as readFile4, readdir as readdir2, rm as rm2, stat as stat2, writeFile as writeFile4 } from "node:fs/promises";
...
async function listBackups() {
  ...
  for (const name2 of names) {
    try {
      const s = await stat2(join6(dir, name2));
      list.push({ name: name2, size: s.size, createdAt: createdAtOf(name2), format: formatOf(name2) });
    } catch {
    }
  }
...
async function pruneBackups(retention) {
  ...
  for (const b of list.slice(keep)) {
    try {
      await rm2(join6(dir, b.name), { force: true });
    } catch {
    }
  }
}
...
async function restoreBackup(name2) {
  if (!FILE_RE.test(name2)) {
    throw new Error("invalid backup name");
  }
  const file = join6(backupDir(), name2);
  const st = await stat2(file).catch(() => null);
  if (!st || !st.isFile()) {
  ...
  if (format === "db") {
    ...
    await rm2(`${dbPath()}-wal`, { force: true }).catch(() => {
    });
    await rm2(`${dbPath()}-shm`, { force: true }).catch(() => {
    });
    reopenDb();
...
async function deleteBackup(name2) {
  ...
  await rm2(join6(backupDir(), name2), { force: true });
  return true;
}
...
// src/host/services/persona/persona-service.ts
import { existsSync as existsSync2, readFileSync as readFileSync6, readdirSync as readdirSync2 } from "node:fs";
...
function listScopeTree() {
  const nodes = [];
  let raw = {};
  if (existsSync2(workspaceStorePath())) {
    try {
      raw = JSON.parse(readFileSync6(workspaceStorePath(), "utf8"));
    } catch {
    }
  }
  ...
  if (kind === "workspace") {
    try {
      ...
      for (const e of entries) {
        ...
        const childPath = join7(absPath, e.name);
...
// src/host/services/session-scope/session-scope.ts
...
function sortSessionsRecursively(node) {
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (method === "POST" && segments[0] === "export" && segments[1] === "save") {
        ...
        let n = 1;
        while (existsSync3(join8(dir, finalName))) {
          finalName = ext ? `${base} (${n}).${ext}` : `${base} (${n})`;
          n++;
        }
        const target = join8(dir, finalName);
      ...
      if (method === "POST" && tail === "/skills/export/entries") {
        ...
        if (scope === "private") {
          ...
          const exportRoot = scope === "project" ? join8(projectRoot, ".dsh", "skills") : void 0;
      ...
      if (method === "GET" && segments[0] === "assets" && segments[1] === "whale" && segments.length === 2) {
        try {
          ...
          const buf = await readFile5(fileURLToPath(fileUrl));
          res.writeHead(200, {
...
## \u63D2\u4EF6\u5B9A\u4F4D
\u4F60\u5E26\u300C\u8BCD\u5E93\u300D\u63D2\u4EF6\uFF1A\u4E00\u4E2A\u53EF\u6301\u7EED\u4FDD\u5B58\u3001\u68C0\u7D22\u3001\u4F18\u5316\u3001\u7EDF\u8BA1\u7684\u53EF\u590D\u7528\u63D0\u793A\u8BCD\u5E93\u3002
...
- \u659C\u6760\u547D\u4EE4 /prompts\uFF08\u5927\u5C0F\u5199\u4E0D\u654F\u611F\uFF0C\u652F\u6301\u7F29\u5199\u522B\u540D\uFF09\uFF1A
  ...
  - -h\uFF1A\u67E5\u770B\u5B8C\u6574\u4F7F\u7528\u624B\u518C
- \u754C\u9762\u64CD\u4F5C\uFF1A\u4FA7\u8FB9\u680F\u8BCD\u5E93\u4E0E\u804A\u5929\u6846\u6309\u94AE\uFF0C\u53EF\u63D2\u5165 / \u590D\u5236 / \u6DA6\u8272\u63D0\u793A\u8BCD\uFF1B\u542B\u5360\u4F4D\u7B26\u53D8\u91CF\u7684\u6A21\u677F\u652F\u6301\u53D8\u91CF\u586B\u5145\u3002
...
async function ensureHarnessFile() {
  try {
    await readFile6(harnessPath(), "utf8");
  } catch {
    await mkdir5(dirname5(harnessPath()), { recursive: true });
    await writeFile6(harnessPath(), DEFAULT_HARNESS, "utf8");
  }
}
...
function harnessSystemSync() {
  ...
  let content = "";
  try {
    content = stripBom(readFileSync7(harnessPath(), "utf8")).trim();
  } catch {
...
var manualZh = [
  ...
  "- /prompts -v\uFF1A\u8F93\u51FA\u5168\u90E8\u5386\u53F2\u7248\u672C\u7684\u66F4\u65B0\u8BF4\u660E\uFF0C\u6309\u5F53\u524D\u754C\u9762\u8BED\u8A00\u5C55\u793A\uFF08\u522B\u540D -version\uFF09",
...
var manualEn = [
  ...
  "- /prompts -v: output the full history of release notes in your interface language (alias -version)",
