...
function soulSystemSync() {
  ...
  const cached2 = soulCache;
  if (cached2 && (meta !== null && cached2.meta !== null && cached2.meta.mtimeMs === meta.mtimeMs && cached2.meta.size === meta.size || meta === null && cached2.meta === null)) {
...
// src/host/services/announcement.ts
var DEFAULT_ANNOUNCEMENT_URL = "https://gitee.com/superBigYo/dsh-prompt-library/raw/master/announcement.json";
...
function cleanMarkdown(text) {
  return text.replace(/!\[[^\]]*\]\([^)]*\)/g, "").replace(/\[([^\]]*)\]\([^)]*\)/g, "$1").replace(/^\s{0,3}#{1,6}\s+/gm, "").replace(/\*\*([^*]+)\*\*/g, "$1").replace(/~~([^~]+)~~/g, "$1").replace(/`([^`]+)`/g, "$1").replace(/```/g, "").replace(/^[\s]*[-*+]\s+/gm, "\u2022 ").replace(/^[\s]*(\d+)\.\s+/gm, "$1. ").replace(/\n{3,}/g, "\n\n").trim();
}
function parseRemote(text) {
  try {
    const obj = JSON.parse(text);
    const manual = Array.isArray(obj.manual) ? obj.manual.filter((x) => typeof x === "string") : void 0;
    const notice = typeof obj.notice === "string" ? obj.notice : void 0;
...
async function getAnnouncement() {
  try {
    ...
    try {
      ...
      const data = { source: "remote", manual, notice };
...
function harnessSystemSync() {
  ...
  const cached2 = harnessCache;
  if (cached2 && meta && cached2.meta && cached2.meta.mtimeMs === meta.mtimeMs && cached2.meta.size === meta.size) {
    return cached2.assembled;
  }
  if (cached2 && !meta && !cached2.meta) return cached2.assembled;
  let content = "";
  try {
