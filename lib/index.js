...
async function migrateLegacySettingsIfNeeded() {
  try {
    ...
    if (root && typeof root === "object" && !Array.isArray(root) && root[SETTINGS_NAMESPACE]) {
      return;
    }
  } catch {
  }
  ...
  nextRoot[SETTINGS_NAMESPACE] = stripPersistExcluded(legacy);
  mkdirSync(dirname(pluginSettingsPath()), { recursive: true });
  await writeFile(pluginSettingsPath(), JSON.stringify(nextRoot, null, 2), "utf8");
  try {
    const text = await readFile(systemSettingsPath(), "utf8");
    const sysRoot = load(stripBom(text));
    if (sysRoot && typeof sysRoot === "object" && !Array.isArray(sysRoot) && sysRoot[SETTINGS_NAMESPACE] !== void 0) {
      delete sysRoot[SETTINGS_NAMESPACE];
...
async function readSettingsRaw() {
  await migrateLegacySettingsIfNeeded().catch(() => void 0);
