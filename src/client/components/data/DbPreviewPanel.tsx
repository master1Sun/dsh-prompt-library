...
export function DbPreviewPanel(props: { t?: PLTranslate }): ReactNode {
  ...
  const [devPwBusy, setDevPwBusy] = useState(false);
  ...
  // 校验密码后再开启（明文只发往服务端做摘要比对，不在客户端保存）
  const onDevPwConfirm = () => {
    if (!devPw || devPwBusy) return;
    setDevPwBusy(true);
    setDevPwWrong(false);
    verifyDbDevPassword(devPw)
      .then((ok) => {
        if (ok) {
          setDevPwOpen(false);
          setDevMode(true);
      ...
      .catch(() => setDevPwWrong(true))
      .finally(() => setDevPwBusy(false));
  };
