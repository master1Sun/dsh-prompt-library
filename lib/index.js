...
var GITHUB_API_URL = "https://api.github.com/repos/master1Sun/dsh-prompt-library/releases";
...
function buildVersionLogCopy(lang) {
  ...
  if (zh) {
    return {
      check: (cur, npm, ghO, ghT, exp) => `\u7248\u672C\u68C0\u67E5 \u5F53\u524D=${cur} npm=${npm} git\u6B63\u5F0F=${ghO} git\u6D4B\u8BD5=${ghT} \u4F53\u9A8C\u8BA1\u5212=${exp ? on : off}`,
      noSource: "\u7248\u672C\u68C0\u67E5 \u65E0\u53EF\u7528\u66F4\u65B0\u6E90\uFF0C\u8DF3\u8FC7",
      expResult: (latest, hasUpdate, hasBeta, beta) => `\u7248\u672C\u68C0\u67E5 \u4F53\u9A8C\u8BA1\u5212\u7ED3\u679C latest=${latest} hasUpdate=${bool(hasUpdate)} hasBeta=${bool(hasBeta)} betaLatest=${beta}`,
      normalResult: (latest, hasUpdate, hasBeta, beta) => `\u7248\u672C\u68C0\u67E5 \u666E\u901A\u7ED3\u679C latest=${latest} hasUpdate=${bool(hasUpdate)} hasBeta=${bool(hasBeta)} betaLatest=${beta}`,
      upgradeStart: (target, cmd) => `\u5F00\u59CB\u5347\u7EA7 \u76EE\u6807=${target} \u547D\u4EE4=${cmd}`,
      upgradeFail: (out) => `\u5347\u7EA7\u5931\u8D25 ${out}`,
      upgradeOk: "\u5347\u7EA7\u6210\u529F\uFF0C\u5DF2\u6E05\u9664\u7248\u672C\u68C0\u67E5\u7F13\u5B58",
      silentTo: (v) => `\u9759\u9ED8\u81EA\u52A8\u66F4\u65B0 \u5347\u7EA7\u5230 ${v}`,
      silentSkip: (hasUpdate, hasBeta, latest) => `\u9759\u9ED8\u81EA\u52A8\u66F4\u65B0 \u65E0\u9700\u81EA\u52A8\u5347\u7EA7 (hasUpdate=${bool(hasUpdate)} hasBeta=${bool(hasBeta)} latest=${latest})`,
      silentErr: (msg) => `\u9759\u9ED8\u81EA\u52A8\u66F4\u65B0 \u5F02\u5E38 ${msg}`
    };
  }
  return {
    check: (cur, npm, ghO, ghT, exp) => `Version check current=${cur} npm=${npm} git-official=${ghO} git-test=${ghT} experience=${exp ? on : off}`,
    noSource: "Version check no available update source, skipped",
    expResult: (latest, hasUpdate, hasBeta, beta) => `Version check experience result latest=${latest} hasUpdate=${bool(hasUpdate)} hasBeta=${bool(hasBeta)} betaLatest=${beta}`,
    normalResult: (latest, hasUpdate, hasBeta, beta) => `Version check normal result latest=${latest} hasUpdate=${bool(hasUpdate)} hasBeta=${bool(hasBeta)} betaLatest=${beta}`,
    ...
    silentSkip: (hasUpdate, hasBeta, latest) => `Silent auto-update no upgrade needed (hasUpdate=${bool(hasUpdate)} hasBeta=${bool(hasBeta)} latest=${latest})`,
...
async function fetchGithubVersions() {
  ...
  const result = { official: "", officialTag: "", test: "", testTag: "" };
...
async function checkUpdate(force = false) {
  ...
  if (!npm && !ghOfficial && !ghTest) {
    ...
    const info2 = { current, latest: current, hasUpdate: false, betaLatest: current, hasBeta: false, gitTag: "", betaTag: "" };
    cache = { at: now, info: info2, ttl: CACHE_MS / 2 };
    return info2;
  }
  if (betaEnabled) {
    const stable = (() => {
    ...
    const hasUpdate2 = !!stable && compareVersions(stable, current) > 0;
    const hasBeta2 = !!ghTest && compareVersions(ghTest, current) > 0 && compareVersions(ghTest, stable || current) > 0;
    const latest2 = hasUpdate2 ? stable : hasBeta2 ? ghTest : current;
    const stableIsGit = !!ghOfficial && (!npm || compareVersions(ghOfficial, npm) > 0);
    const info2 = {
      current,
      latest: latest2,
      hasUpdate: hasUpdate2,
      betaLatest: hasBeta2 ? ghTest : latest2,
      hasBeta: hasBeta2,
      // 静默安装的目标（latest）若来自 GitHub（正式或测试版），带上其 tag 改走 git 命令；来自 npm 则留空走 npm
      gitTag: hasUpdate2 ? stableIsGit ? gh?.officialTag || "" : "" : hasBeta2 ? gh?.testTag || "" : "",
      // 红点手动更新的测试版：GitHub 预发布 → git 命令
      betaTag: hasBeta2 ? gh?.testTag || "" : ""
    };
    logVersion(vlog.expResult(latest2, hasUpdate2, hasBeta2, info2.betaLatest));
    cache = { at: now, info: info2, ttl: CACHE_MS };
    return info2;
  }
  const ghAny = compareVersions(ghOfficial, ghTest) >= 0 ? ghOfficial : ghTest;
  const hasBeta = npm && ghAny ? compareVersions(ghAny, npm) > 0 && compareVersions(ghAny, current) > 0 : !!ghAny && compareVersions(ghAny, current) > 0;
