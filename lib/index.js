...
import { cp, mkdir as mkdir3, readFile as readFile4, readdir as readdir3, rename, rm as rm4, stat as stat3, writeFile as writeFile4 } from "node:fs/promises";
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        ...
        try {
          ...
          const text = await readFile4(full, "utf8");
          const lines = text.split("\n");
          let perFile = 0;
...
`);
      ...
      if (method === "POST" && segments[0] === "preview" && segments[1] === "save" && segments.length === 2) {
        ...
        try {
          await writeFile4(path, data.content, "utf8");
      ...
      if (method === "POST" && segments[0] === "preview" && segments[1] === "mkdir" && segments.length === 2) {
        ...
        try {
          await mkdir3(join8(dir, b.name), { recursive: false });
          return json(res, 200, { ok: true, data: { success: true } });
        } catch (err) {
          return json(res, 500, {
            ok: false,
      ...
      if (method === "POST" && segments[0] === "preview" && segments[1] === "newfile" && segments.length === 2) {
        ...
        try {
          await writeFile4(join8(dir, b.name), "", "utf8");
          return json(res, 200, { ok: true, data: { success: true } });
        } catch (err) {
          return json(res, 500, {
            ok: false,
      ...
      if (method === "POST" && segments[0] === "preview" && segments[1] === "rootmtime" && segments.length === 2) {
        ...
        try {
          ...
          const rootStat = await stat3(dir);
