...
import { mkdir as mkdir5, readFile as readFile6, readdir as readdir3, stat as stat4, writeFile as writeFile6 } from "node:fs/promises";
...
var DEFAULT_SETTINGS = {
  ...
  // 是否显示官方右侧栏「词库」面板 tab（关闭后右侧栏不再出现词库入口）
...
async function buildStatus(lang) {
...
function attachAssistantStream(session) {
  ...
  session.onMessage((raw) => {
    ...
    const next = message.lang.toLowerCase().startsWith("en") ? "en" : "zh";
    if (next === lang) return;
    lang = next;
    pushAll();
  });
...
function emitExportDownload(name2, json2) {
  return broadcast({ type: "export-download", name: name2, json: json2 }) > 0;
}
...
function pluginGithubCachePath() {
...
async function enforceMaxCount(maxCount, autoLearnTag) {
  ...
  const rm5 = cur.prepare("DELETE FROM prompts WHERE id = ?");
  for (const { id } of candidates.slice(0, toRemove)) rm5.run(id);
}
...
function deleteTrash(ids) {
  try {
    ...
    if (list.length === 0) return Promise.resolve(0);
    const cur = getDb();
    const rm5 = cur.prepare("DELETE FROM trash WHERE id = ?");
    let deleted = 0;
    cur.exec("BEGIN");
    try {
      for (const id of list) {
        deleted += Number(rm5.run(id).changes);
      }
      cur.exec("COMMIT");
    } catch (e) {
      cur.exec("ROLLBACK");
...
function hashDevPassword(plain) {
  return createHash2("sha256").update(String(plain ?? "")).digest("hex");
}
...
async function queryDeepSeekBalance(apiKey) {
  ...
  let credit = null;
  try {
    const body = await new Promise((resolve2) => {
      const req = httpsGet(
        ...
        (res) => {
          ...
          res.on("end", () => {
            if (code < 200 || code >= 300) return resolve2(null);
            try {
...
import { appendFileSync as appendFileSync2, mkdirSync as mkdirSync3, readFileSync as readFileSync2, statSync } from "node:fs";
...
// src/host/plugin-market.ts
import { get as httpsGet3 } from "node:https";
import { mkdir as mkdir3, readFile as readFile4, writeFile as writeFile4 } from "node:fs/promises";
import { dirname as dirname5 } from "node:path";
var HOT_LIST_URL = "https://dsh-plugin.org/plugins";
var CACHE_TTL_MS = 6 * 60 * 60 * 1e3;
var TOP_N = 8;
var LIST_TIMEOUT_MS = 3e4;
var GITHUB_TIMEOUT_MS = 12e3;
var GITHUB_TTL_MS = 30 * 60 * 1e3;
var BUILTIN_ITEMS = [
  {
    ...
    const req = httpsGet3(
      ...
      (res) => {
        if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
          ...
          fetchPage(next, timeoutMs).then(resolve2, reject);
          return;
        }
...
function recordFromWindow(win, name2) {
  const repoM = fieldOf(win, "repo").includes("/") ? fieldOf(win, "repo") : `${fieldOf(win, "ownerSlug")}/${name2}`;
  if (!name2 || !repoM.includes("/")) return null;
  return {
    id: repoM,
    name: name2,
    desc: descOf(win),
...
function parseHotRecords(html) {
  ...
  while (items.length < TOP_N) {
    ...
    const win = html.slice(idx, idx + 12e3);
    if (!win.includes("stargazers_count")) continue;
    const name2 = fieldOf(win, "slug");
    const p = recordFromWindow(win, name2);
    if (!p) continue;
...
var PINNED_SLUGS = ["dsh-file-workbench-lib", "dsh-prompt-library", "dsh-QQbot"];
var PINNED_REPOS = [
  "master1Sun/dsh-file-workbench-lib",
  "master1Sun/dsh-prompt-library",
  "master1Sun/dsh-QQbot"
];
function parsePinnedRecords(html) {
  const out = [];
...
async function fetchGithubInfo(repo) {
  try {
    const meta = await fetchJson2(`https://api.github.com/repos/${repo}`);
    let version = "";
    try {
      const rel = await fetchJson2(
        `https://api.github.com/repos/${repo}/releases/latest`
      );
      version = bareVersion(rel.tag_name ?? "");
    } catch {
      try {
        const tags = await fetchJson2(
...
async function readGithubCache() {
  try {
    const raw = await readFile4(pluginGithubCachePath(), "utf8");
    const parsed = JSON.parse(raw);
...
async function writeGithubCache(cache2) {
  try {
    const p = pluginGithubCachePath();
    await mkdir3(dirname5(p), { recursive: true });
    await writeFile4(p, JSON.stringify(cache2), "utf8");
  } catch {
  }
}
var githubInflight = null;
async function doFetchGithub(force) {
  const cached = await readGithubCache();
  if (!force && cached && Date.now() - cached.fetchedAt < GITHUB_TTL_MS) return cached;
  const entries = await Promise.all(
    PINNED_REPOS.map(
      async (repo) => [repo, await fetchGithubInfo(repo)]
    )
  );
  ...
  const out = { fetchedAt: Date.now(), items };
  await writeGithubCache(out);
  return out;
}
function fetchPinnedGithub(force) {
  if (!githubInflight) {
...
async function readCache() {
  try {
    const raw = await readFile4(pluginMarketCachePath(), "utf8");
    const parsed = JSON.parse(raw);
...
async function writeCache(cache2) {
  try {
...
async function fetchMarketList(force, cached) {
  ...
  try {
    ...
    const cache2 = { fetchedAt: Date.now(), items };
    await writeCache(cache2);
    return { source: "live", fetchedAt: cache2.fetchedAt, items };
  } catch (err) {
...
function fetchPluginMarket(force = false) {
  if (!inflight) {
...
// src/host/backup.ts
import { copyFile, mkdir as mkdir4, readFile as readFile5, readdir as readdir2, rm as rm4, stat as stat3, writeFile as writeFile5 } from "node:fs/promises";
...
async function listBackups() {
  ...
  for (const name2 of names) {
    try {
      const s = await stat3(join6(dir, name2));
...
async function pruneBackups(retention) {
  ...
  for (const b of list.slice(keep)) {
    try {
      await rm4(join6(dir, b.name), { force: true });
    } catch {
    }
  }
}
async function runBackup(retention, format = "db") {
  ...
  if (format === "json") {
    ...
    await writeFile5(target, JSON.stringify(data, null, 2), "utf8");
  } else {
    checkpointDb();
...
async function restoreBackup(name2) {
  ...
  if (format === "db") {
    ...
    const raw = JSON.parse(await readFile5(file, "utf8"));
...
async function deleteBackup(name2) {
  ...
  await rm4(join6(backupDir(), name2), { force: true });
  return true;
}
...
async function listFsDirectory(input) {
  ...
  const st = await stat4(dir);
  if (!st.isDirectory()) throw new Error(`not a directory: ${dir}`);
  const all = await readdir3(dir, { withFileTypes: true });
  const dirs = all.filter((d) => {
    if (d.isDirectory()) return true;
    if (d.isSymbolicLink()) {
      try {
        return statSync2(join8(dir, d.name)).isDirectory();
      } catch {
        return false;
      }
    }
...
async function createFsDirectory(parent, name2) {
  ...
  const target = join8(base, clean);
  await mkdir5(target, { recursive: false });
  return target;
}
    ...
    try {
      ...
      if (method === "GET" && segments[0] === "assets" && segments[1] === "whale" && segments.length === 2) {
        try {
          ...
          const buf = await readFile6(fileURLToPath2(fileUrl));
          res.writeHead(200, {
